...
void tytul(void) {
	clrscr();gotoxy(10,1);printf("INSTYTUT TELEKOMUNIKACJI I AKUSTYKI");
	printf(" POLITECHNIKI WROCAWSKIEJ");gotoxy(29,3);printf("KODOWANIE\t(PROJEKT)");
	gotoxy(17,10);printf("KOMPUTEROWA SYMULACJA PRACY KANAU KODOWEGO");
	gotoxy(16,11);printf("ZAWIERAJ¤CEGO KODER I DEKODER KODU SPLOTOWEGO");
	gotoxy(22,12);printf("(przesyˆanie ci¥gu znak¢w ASCII)");
	gotoxy(1,20);printf("Prowadz¥cy: dr in¾. Paweˆ Kabacik");
	gotoxy(3,21);printf("Wykonali: Piotr Borowiec, Paweˆ Pelc");
	getch();
}
int text2bin(void) {  
	char text[200];
	...
	clrscr();
	printf("Podaj ci¥g znak¢w ASCII: (max.200)\n\n");
	gets(text);
	l=strlen(text);
	for (i=0; i<l; i++) {
		for (j=0; j<8; j++) {
			printf("%d",(nn[8*i+j]=text[i]%2));
			text[i]=text[i]/2;
		}
	}
	return l;
